...
export const uploadCommentImage = multer({
  storage: multerS3({
    ...
    key: function (req, file, cb) {
      const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9)
      const ext = path.extname(file.originalname)
      cb(null, `comment-images/${uniqueSuffix}${ext}`)
    }
  }),
  fileFilter: fileFilter,
  limits: {
...
// Multer upload configuration for comment reply images
export const uploadCommentReplyImage = multer({
  storage: multerS3({
    s3: s3Client,
    bucket: config.aws.s3BucketName,
    contentType: multerS3.AUTO_CONTENT_TYPE,
    ...
    key: function (req, file, cb) {
      const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9)
      const ext = path.extname(file.originalname)
      cb(null, `comment-reply-images/${uniqueSuffix}${ext}`)
    }
  }),
  fileFilter: fileFilter,
  limits: {
